...
const ApolloClientPlugin = createPlugin({
  deps: {
    ...
    wsEndpoint: ApolloClientWsEndpointToken.optional,
    fetch: FetchToken,
  ...
  provides({
    ...
    return (ctx, initialState) => {
      ...
      const context = schema && __NODE__ && typeof apolloContext === 'function'
        ? apolloContext(ctx)
        : apolloContext;

      const httpLink = schema && __NODE__ ? null : new HttpLink({
        uri: endpoint,
      ...
      if (__BROWSER__) {
        ...
        if (wsEndpoint) {
          ...
          requestLink = split(isSubscriptionOperation, wsLink, httpLink);
        } else {
